...
mmap_stream* map_file(const char* filename, AAssetManager* mgr) {
  ...
  if(fd == -1) {
    __android_log_print(ANDROID_LOG_VERBOSE, "mmap_stream_android", "Could not open asset %s. Have you specified this as noCompress in build.gradle?", filename);
    return nullptr;
  } else {
  ...
  __android_log_print(ANDROID_LOG_VERBOSE, "mmap_stream_android", "Created stream from %p to %p", stream_start, stream_end);
